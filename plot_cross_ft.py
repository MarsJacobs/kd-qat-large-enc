...
def main():
    ...
    # Load pt Data File
    for i in range(12):
        FP_input_dict[f"input_{i}"] = torch.load(folder_name+f"/o_pths/Q_layer_{i}_ffn1_input.pt")
        FP_output_dict[f"output_{i}"] = torch.load(folder_name+f"/o_pths/Q_layer_{i}_ffn2_output.pt")
        FP_Loutput_dict[f"Loutput_{i}"] = torch.load(folder_name+f"/o_pths/Q_layer_{i}_ffn2_Layernorm_output.pt")
    ...
    for i in range(12):
        ...
        for word in range(seq_length):
            ...
            ax1.hist(FP_input_dict[f"input_{i}"][sentence_idx][word].reshape(-1).detach().cpu().numpy(), bins=100, label="input", color="dimgray")
            ax1.set_title(f"{i}th Layer Word input : " + word_name)
            
            ax2.hist(FP_output_dict[f"output_{i}"][sentence_idx][word].reshape(-1).detach().cpu().numpy(), bins=100, label="output", color="dimgray")
            ax2.set_title(f"{i}th Layer Word output : " + word_name)
            
            ax3.hist(FP_Loutput_dict[f"Loutput_{i}"][sentence_idx][word].reshape(-1).detach().cpu().numpy(), bins=100, label="Loutput", color="dimgray")
        ...
        ax1.plot(x_axis, F_in_min_axis, 'o-', color = 'dimgray', label = 'min_old')
        ax1.plot(x_axis, F_in_max_axis, 'o-', color = 'dimgray', label = 'max_old')
        ax1.plot(x_axis, F_in_mean_axis, color = 'dimgray', label = 'mean_old')
        ...
        ax2.plot(x_axis, F_o_min_axis, 'o-', color = 'dimgray', label = 'min_old')
        ax2.plot(x_axis, F_o_max_axis, 'o-', color = 'dimgray', label = 'max_old')
        ax2.plot(x_axis, F_o_mean_axis, color = 'dimgray', label = 'mean_old')
        ...
        ax3.plot(x_axis, F_l_min_axis, 'o-', color = 'dimgray', label = 'min_old')
        ax3.plot(x_axis, F_l_max_axis, 'o-', color = 'dimgray', label = 'max_old')
        ax3.plot(x_axis, F_l_mean_axis, color = 'dimgray', label = 'mean_old')
