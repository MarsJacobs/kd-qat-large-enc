...
CUDA_VISIBLE_DEVICES=$1 python quant_task_glue_kl.py --data_dir data --task_name $2 --output_dir output --num_train_epochs 3 --bert ${bert} \
...
--attn_kl_distill $attn_kl_distill --output_coeff ${output_coeff} --context_coeff ${context_coeff} --val_coeff ${val_coeff} --attnmap_coeff ${attnmap_coeff} --cls_coeff ${cls_coeff} --att_coef ${att_coeff} --rep_coeff ${rep_coeff} --word_coeff ${word_coeff} \
