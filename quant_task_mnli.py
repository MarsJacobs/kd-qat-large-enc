    ...
    for epoch_ in range(int(num_train_epochs)):
        ...
        for batch in tqdm(train_dataloader,desc=f"Epoch_{epoch_}", mininterval=0.01, ascii=True):
            ...
            if args.attnmap_distill:
                ...
                l_attmap_loss.update(kld_loss_mean.item())
                attmap_loss = args.attnmap_coeff*kld_loss_mean
            ...
            if args.val_distill:
                ...
                # extended_attention_mask = input_mask.unsqueeze(1).unsqueeze(3)
                val_mask = (1.0 - mask) * -10000.0
                

                student_value = student_values[-1][1]
                student_value = torch.matmul(student_value, student_value.transpose(-1, -2))
                teacher_value = teacher_values[-1][1]
                teacher_value = torch.matmul(teacher_value, teacher_value.transpose(-1, -2))

                val_student = torch.nn.Softmax(dim=-1)(student_value + val_mask)
                val_teacher = torch.nn.Softmax(dim=-1)(teacher_value + val_mask)

                val_student = torch.clamp_min(val_student, 1e-8)
                val_teacher = torch.clamp_min(val_teacher, 1e-8)

                 # p(t) log p(s) = negative cross entropy
                neg_cross_entropy = val_teacher * torch.log(val_student) * mask
                neg_cross_entropy = torch.sum(neg_cross_entropy, dim=-1)  # (b, h, s, s) -> (b, h, s)
                neg_cross_entropy = torch.sum(neg_cross_entropy, dim=-1) / seq_lengths.view(-1, 1)  # (b, h, s) -> (b, h)

                # p(t) log p(t) = negative entropy
                neg_entropy = val_teacher * torch.log(val_teacher) * mask
                neg_entropy = torch.sum(neg_entropy, dim=-1)  # (b, h, s, s) -> (b, h, s)
                neg_entropy = torch.sum(neg_entropy, dim=-1) / seq_lengths.view(-1, 1)  # (b, h, s) -> (b, h)
                
                val_loss = neg_entropy - neg_cross_entropy
                
                val_loss_mean = torch.mean(val_loss)
                
                
                l_val_loss.update(val_loss_mean.item())
                val_loss = args.val_coeff * val_loss_mean
